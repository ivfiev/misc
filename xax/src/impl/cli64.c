...
int use_stdin(void) {
  return !strcmp(args_get("arg2"), "--stdin");
}
...
void info64(void) {
  ...
  for (uintptr_t ptr = addr + (int)from * 4; ptr <= addr + (int)to * 4; ptr += 4) {
    ...
    read_mem_bytes(fd, ptr, word.bytes, 8);
    off += snprintf(line, SIZEARR(line), "0x%lx  ", ptr);
    if (is_ptr64(word, ds, ds_size)) {
    ...
    if (is_float32(word)) {
      off += snprintf(line + off, SIZEARR(line) - off, "\t\t[%f]", word.float32);
    }
    if (ptr == addr) {
...
static void init(void) {
  args_add("range64", range64);
  args_add("delta64", delta64);
  args_add("info64", info64);
  args_add("ptr_scan64", ptr_scan64);
}