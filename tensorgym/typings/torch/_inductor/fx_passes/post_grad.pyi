...
from ..pattern_matcher import Arg, CallFunction, Ignored, KeywordArg, ListOf, MULTIPLE, Match, init_once_fakemode, register_graph_pattern
...
def prepare_softmax_extra_check(match) -> Any | Literal[False]:
    """
...
def register_lowering_pattern(pattern, extra_check=..., pass_number=...) -> Callable[[Callable[_P, _T]], Callable[_P, _T]]:
    """
...
@register_lowering_pattern(CallFunction(aten.add, CallFunction(aten.mm, KeywordArg("mat1"), KeywordArg("mat2")), CallFunction(aten.mm, KeywordArg("mat3"), KeywordArg("mat4"))), extra_check=is_valid_mm_plus_mm)
...
def same_meta(node1: torch.fx.Node, node2: torch.fx.Node) -> Any | Literal[False]:
...
def apply_pass_to_subgraphs(pass_fn: Callable[[fx.Graph], None], graph: fx.Graph) -> None:
...
@register_graph_pattern(CallFunction(aten.addmm, KeywordArg("inp"), Arg(), Arg(), beta=KeywordArg("beta"), alpha=KeywordArg("alpha")), pass_dict=pass_patterns[2], extra_check=should_prefer_unfused_addmm)
def unfuse_bias_add_to_pointwise(match: Match, mat1, mat2, *, inp, alpha, beta) -> None:
    ...
