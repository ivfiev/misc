...
def joint_graph_passes(graph: torch.fx.GraphModule, input_device: Optional[torch.device] = ...) -> GraphModule:
    """
...
@register_graph_pattern(CallFunction(torch.ops.aten.view.default, KeywordArg("arg"), KeywordArg("size")), pass_dict=early_patterns)
def pointless_view(match: Match, arg, size) -> None:
    """Remove no-op view"""
    ...

@register_graph_pattern(CallFunction(aten.view.default, CallFunction(aten.view.default, KeywordArg("arg"), KeywordArg("size1")), KeywordArg("size2")), pass_dict=early_patterns)
...
@register_graph_pattern(CallFunction(aten.permute.default, CallFunction(aten.permute.default, KeywordArg("arg"), KeywordArg("perm1")), KeywordArg("perm2")), pass_dict=early_patterns)
...
@register_graph_pattern(CallFunction(aten.scatter.value, CallFunction(aten.full, KeywordArg("shape"), KeywordArg("background_val"), dtype=KeywordArg("dtype")), KeywordArg("dim"), KeywordArg("selector"), KeywordArg("val")), pass_dict=patterns, extra_check=scatter_upon_const_tensor_extra_check)
def scatter_upon_const_tensor(match: Match, shape, background_val, dtype, dim, selector, val) -> None:
    """
