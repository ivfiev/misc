...
class CustomPartitionerFn(ABC):
    ...
    """
    @abstractmethod
    def __call__(self, gm: torch.fx.GraphModule, joint_inputs: Sequence[object], **kwargs: Any) -> tuple[torch.fx.GraphModule, torch.fx.GraphModule]:
        """
...
class CustomKnapsackSolver(ABC):
    ...
    ** IMPORTANT ** If your custom knapsack solver's behavior depends on some external state,
    ...
    """
    @abstractmethod
    def __call__(self, memory: list[float], joint_graph: torch.fx.Graph, max_memory: float, node_info: NodeInfo, all_recomputable_banned_nodes: list[torch.fx.Node]) -> tuple[list[int], list[int]]:
        """
