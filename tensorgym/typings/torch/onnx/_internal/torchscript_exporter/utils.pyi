...
@deprecated("Please remove usage of this function. Copy its logic if it is required in user code", category=None)
@contextlib.contextmanager
def disable_apex_o2_state_dict_hook(model: torch.nn.Module | torch.jit.ScriptFunction) -> Generator[None, Any, None]:
...
@deprecated("The feature will be removed. Please remove usage of this function " "and implement equivalent logic if needed", category=None)
@contextlib.contextmanager
def exporter_context(model, mode: _C_onnx.TrainingMode, verbose: bool) -> Generator[tuple[None, None, None], Any, None]:
    """A context manager to temporarily set the training mode of ``model``
...
def export(model: torch.nn.Module | torch.jit.ScriptModule | torch.jit.ScriptFunction, args: tuple[Any, ...] | torch.Tensor, f: str, *, kwargs: dict[str, Any] | None = ..., export_params: bool = ..., verbose: bool = ..., training: _C_onnx.TrainingMode = ..., input_names: Sequence[str] | None = ..., output_names: Sequence[str] | None = ..., operator_export_type: _C_onnx.OperatorExportTypes = ..., opset_version: int | None = ..., do_constant_folding: bool = ..., dynamic_axes: Mapping[str, Mapping[int, str]] | Mapping[str, Sequence[int]] | None = ..., keep_initializers_as_inputs: bool | None = ..., custom_opsets: Mapping[str, int] | None = ..., export_modules_as_functions: bool | Collection[type[torch.nn.Module]] = ..., autograd_inlining: bool = ...) -> None:
...
_qtype_vtype_map = ...
def unpack_quantized_tensor(value, cast_onnx_accepted=...) -> tuple[Tensor, Tensor, Tensor] | tuple[Any | Tensor]:
    ...

@deprecated("Unconvertible ops are not definitive. Please remove usage of this function")
def unconvertible_ops(model, args, training: _C_onnx.TrainingMode = ..., opset_version: int | None = ...) -> tuple[_C.Graph, list[str]]:
...
def register_custom_op_symbolic(symbolic_name: str, symbolic_fn: Callable, opset_version: int) -> None:
