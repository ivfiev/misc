...
def calculate_quantization_scaling(graph: torch.fx.Graph, node: torch.fx.Node, max: float = ..., min: float = ..., position: int = ...) -> torch.fx.Node:
    ...

def perform_quantization(graph: torch.fx.Graph, node: torch.fx.Node, scale_node: torch.fx.Node, quant_type: torch.dtype, clamp_min: float, clamp_max: float, position: int) -> torch.fx.Node:
    ...
...
def perform_fp8_activation_quantization(fwd_module: fx.GraphModule, bwd_module: fx.GraphModule, bwd_module_inputs: dict[str, fx.Node]) -> None:
    ...

def enable_activation_quantization(saved_values: list[fx.Node], fwd_module: fx.GraphModule, bwd_module: fx.GraphModule, static_lifetime_input_nodes: OrderedSet[fx.Node] | None = ...) -> None:
    ...

def default_partition(joint_module: fx.GraphModule, _joint_inputs: Any, *, num_fwd_outputs: int, static_lifetime_input_indices: list[int] | None = ..., static_lifetime_input_nodes: OrderedSet[fx.Node] | None = ...) -> tuple[fx.GraphModule, fx.GraphModule]:
    """
...
def sort_depths(args: tuple[Any, ...], depth_map: dict[fx.Node, int]) -> list[tuple[fx.Node, int]]:
    ...
...
def apply_graphsafe_rng_functionalization(fw_module: torch.fx.GraphModule, bw_module: torch.fx.GraphModule, fw_node: torch.fx.Node, bw_node: torch.fx.Node, device: torch.device, rng_count: int, last_fwd_input: torch.fx.Node, last_bwd_input: torch.fx.Node) -> tuple[torch.fx.Node, torch.fx.Node]:
    """
...
def solve_min_cut(joint_graph: fx.Graph, node_info: NodeInfo, min_cut_options: MinCutOptions, dont_ban: OrderedSet[fx.Node] | None = ...) -> tuple[list[fx.Node], OrderedSet[fx.Node]]:
    ...

def visualize_min_cut_graph(nx_graph: nx.DiGraph[str, dict[str, Any]]) -> tuple[str | None, str | None]:
...
def choose_saved_values_set(joint_graph: fx.Graph, node_info: NodeInfo, memory_budget: float = ...) -> list[fx.Node]:
    ...

def thread_graphsafe_rng_from_hops(module: fx.GraphModule, is_backward: bool) -> fx.GraphModule:
    """
...
def classify_nodes(joint_module: fx.GraphModule, static_lifetime_input_indices: list[int], num_fwd_outputs: int) -> NodeInfo:
    ...

def min_cut_rematerialization_partition(joint_module: fx.GraphModule, _joint_inputs: Any, compiler: str = ..., *, num_fwd_outputs: int, static_lifetime_input_indices: list[int] | None = ...) -> tuple[fx.GraphModule, fx.GraphModule]:
    """
...
def draw_graph(traced: torch.fx.GraphModule, fname: str, figname: str = ..., clear_meta: bool = ..., prog: str | list[str] | None = ..., parse_stack_trace: bool = ..., dot_graph_shape: str | None = ...) -> None:
    ...
