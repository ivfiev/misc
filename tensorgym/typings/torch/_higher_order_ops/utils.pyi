...
def reenter_make_fx(fn, subgraph_decomp_table=...) -> _Wrapped[..., Any, ..., GraphModule]:
    ...
...
_hop_compile_tls = ...
@contextmanager
def setup_compilation_env() -> Generator[Callable[..., Any] | CompilerFn, Any, None]:
    """
...
def save_values_for_backward(ctx, args) -> None:
    ...

def saved_values(ctx) -> tuple[Any, ...]:
    ...

def split_into_chunks(iterable: Sequence[Any], chunk_sizes: list[int]) -> list[Any]:
    ...

def create_bw_fn(fn: Callable, args: tuple[Any, ...], return_fw_outputs: bool = ...) -> Callable:
    """
...
def check_input_alias_and_mutation_return_outputs(gm: torch.fx.GraphModule) -> tuple[dict[int, int], dict[int, int], dict[int, int], list[int], Union[tuple[Any, ...], list[Any]],]:
    ...
...
def call_op(op: Union[OpOverload, HopInstance], args, kwargs) -> Any:
    ...

def materialize_as_graph(fn: Callable, args: tuple[Any, ...], include_key_set: Optional[torch._C.DispatchKeySet] = ..., exclude_key_set: Optional[torch._C.DispatchKeySet] = ..., force_enable_grad=..., subgraph_decomp_table: Optional[Mapping[OpOverload, Callable]] = ...) -> torch.fx.GraphModule:
    ...
