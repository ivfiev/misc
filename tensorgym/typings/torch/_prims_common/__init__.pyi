...
from typing import Any, NamedTuple, Optional, TYPE_CHECKING, TypeAlias, TypeGuard, TypeVar, Union, cast, overload
...
def compare_tensor_meta(a: TensorLikeType, b: TensorLikeType, check_sizes=..., check_strides=..., *, allow_rhs_unbacked=..., check_conj=...) -> None:
    """
...
def check_significant_strides(a: TensorLikeType, b: TensorLikeType, *, only_cuda=..., allow_rhs_unbacked=...) -> tuple[bool, Optional[int]]:
    ...
...
def compute_elementwise_output_logical_to_physical_perm(*tensors, _skip_checks=..., ambiguity_check=...) -> tuple[list[int], bool]:
    ...
...
def reduction_dtypes(arg, output_dtype_kind: REDUCTION_OUTPUT_TYPE_KIND, dtype: Optional[torch.dtype] = ...) -> tuple[torch.dtype, Optional[torch.dtype]]:
    ...
...
def check_in_bounds_for_storage(a: torch.TypedStorage, shape: ShapeType, strides: StrideType, storage_offset: int) -> None:
    """
    Determines if the given shape, strides, and offset are valid for the given storage.
    """
    ...

@deprecated("`torch._prims_common.check` is deprecated and will be removed in the future. " "Please use `torch._check*` functions instead.", category=FutureWarning)
...
def are_strides_like_channels_last_or_false(shape: Sequence[int], strides: Sequence[int]) -> bool:
    ...
