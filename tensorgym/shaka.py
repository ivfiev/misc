...
class Transformer(nn.Module):
    def __init__(self) -> None:
        ...
        self.blocks = nn.Sequential(*[Block() for _ in range(6)])
        self.norm = nn.LayerNorm(C)
        self.unembed = nn.Linear(C, V)
    ...
    def generate(self, prompt: str, n: int) -> str:
        gen = prompt
        for _ in range(n):
            x = torch.tensor(encode(gen[max(0, len(gen) - T) :])).view(1, -1).to(device)
            logits, _ = self(x)
...
with torch.no_grad():
    print("\nBase model output:")
    print(model.generate("Adam", 1000))
